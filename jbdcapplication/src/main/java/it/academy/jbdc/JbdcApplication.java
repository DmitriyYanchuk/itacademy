...
public class JbdcApplication {
    ...
    public static void main(final String[] args) throws JbdcException {
        try (final Connection connection = DriverManager.getConnection(PROPERTIES.getProperty("url"), PROPERTIES)) {
            ...
            throw new JbdcException("SQL connection error", e);
        }
    }

    private static Properties takeProperties() {
        try (final InputStream inputStream = JbdcApplication.class.getClassLoader().getResourceAsStream("application.properties")) {
